...
Hit Render_World::Closest_Intersection(const Ray& ray)
{
    ...
    for (unsigned int i = 0; i < objects.size(); i++){
        Hit possibleHit = objects.at(i)->Intersection(ray, objects.at(i)->number_parts);
...
vec3 Render_World::Cast_Ray(const Ray& ray,int recursion_depth)
{
    ...
    if (Closest_Intersection(ray).dist >= small_t) {
      ...
      vec3 dummy; //empty vector TA told to do this
      color = background_shader->Shade_Surface(ray,dummy,ray.endpoint,recursion_depth) ;
    }
    return color;
}
